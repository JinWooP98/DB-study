...
-- CHAR타입끼리의 비교
-- 데이터 크기가 서로 다르다면, 작은쪽을 크게 바꾸어 주며 그만큼 뒤에 공백을 생성
-- 그리고 비교한다.
...
SELECT 
    CONCAT('SQL', 'Developer'),   -- 문자열을 결합 == 'SQL' || 'Developer' // 다만, 이 문법은 Oracle에서만 사용가능
    SUBSTR('SQL Developer', 1, 3), -- 문자열 자름 1번부터 3개자름 (첫글자가 1번)
    LENGTH('HELLO WORLD'),         -- 문자열의 길이
    TRIM('    HI   '),-- 좌우 공백 제거
    LTRIM('    HELLOW     '), -- 왼쪽 공백 제거
    RTRIM('        HELLOW         '), -- 오른쪽 공백 제거
    LTRIM('HELLOhello', 'HELL') -- 왼쪽에서 발견된 HE를 제거
FROM dual;
...
SELECT
    ...
    TO_DATE('20240101', 'YYYYMMDD'), -- 시분초를 설정안하면 00시 00분 00초로 기본 세팅,
    TO_DATE('202401', 'YYYYMM'), -- 날짜도 설정안하면 1일로 설정
