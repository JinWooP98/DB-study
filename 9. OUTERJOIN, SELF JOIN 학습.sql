...
-- # LEFT OUTER JOIN
-- 조인되는 왼쪽 테이블은 모두 조회하고, 오른쪽 테이블은 조인조건에 매칭된 것만 조회합니다. // 매칭이 안되면 전부 NULL로 처리한다.
...
-- 사원이 존재하는 부서 14개
-- INNER JOIN의 결과 41건
-- LEFT OUTER JOIN의 결과 46건 -> INNER 41 + 부서가 없는 사원 5
-- RIGHT OUTER JOIN의 결과 43건 -> INNER 41 + 사원이 없는 부서 2
-- FULL OUTER JOIN의 결과 48건 -> INNER 41 + 부서가 없는 사원 5 + 사원이 없는 부서 2
